...
if ($azione === 'elimina' && $id) {
    try {
        ...
        header('Location: fornitori_lista.php?ok=1');
        exit;
...
if ($azione === 'modifica' && $id) {
    ...
    if (!$forn) {
        header('Location: fornitori_lista.php');
        exit;
    }
}
...
<body>
    ...
    <div class="container">
        ...
        <?php if (!empty($_GET['ok'])): ?>
            <div class="success">Operazione completata.</div><?php endif; ?>
            ...
            <form method="POST">
                ...
                <div>
                    ...
                    <input type="text" name="ragione_sociale"
                        value="<?= htmlspecialchars($forn['ragione_sociale'] ?? '') ?>" required>
                </div>
                <div>
                    <label>Partita IVA *</label>
                    <input type="text" name="partita_iva" value="<?= htmlspecialchars($forn['partita_iva'] ?? '') ?>"
                        required>
                </div>
                <div>
                    <label>Email</label>
                    <input type="email" name="email" value="<?= htmlspecialchars($forn['email'] ?? '') ?>">
                </div>
