...
if (!$cli)
    die('Profilo cliente non trovato.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$stmt->fetch()) {
        ...
        try {
            ...
            header('Location: miei_interventi.php?ok=1');
            exit;
...
if ($azione === 'dettaglio' && $id) {
    ...
    if (!$dettaglio) {
        header('Location: miei_interventi.php');
        exit;
    }
...
<body>
    ...
    <div class="container">
        ...
        <?php if (!empty($_GET['ok'])): ?>
            <div class="success">Richiesta inviata! L'officina ti contatterà per confermare.</div><?php endif; ?>
                    ...
                    <tbody>
                        <?php foreach ($interventi as $i): ?>
                            <tr>
                                ...
                                <td><?= $i['data_uscita'] ?? '-' ?></td>
                                <td style="color:<?= $stato_colori[$i['stato']] ?? 'black' ?>">
                                    <strong><?= htmlspecialchars($i['stato']) ?></strong></td>
        ...
        <?php elseif ($azione === 'nuovo'): ?>
            ...
            <?php if (empty($mie_moto)): ?>
                <p>Non hai moto registrate. <a href="mie_moto.php?azione=nuova">Aggiungi una moto</a> prima di richiedere un
                    intervento.</p>
            <?php else: ?>
                <form method="POST">
                    <div>
                        ...
                        <select name="id_moto" required>
                            ...
                            <?php foreach ($mie_moto as $m): ?>
                                <option value="<?= $m['id_moto'] ?>">
                                    <?= htmlspecialchars($m['marca'] . ' ' . $m['modello'] . ' - ' . $m['targa']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
        ...
        <?php elseif ($azione === 'dettaglio' && $dettaglio): ?>
            ...
            <table>
                <tbody>
                    <tr>
                        <th>Moto</th>
                        <td><?= htmlspecialchars($dettaglio['marca'] . ' ' . $dettaglio['modello'] . ' (' . $dettaglio['targa'] . ')') ?>
                        </td>
                    </tr>
                    <tr>
                        <th>Meccanico</th>
                        <td><?= htmlspecialchars($dettaglio['mec_nome'] . ' ' . $dettaglio['mec_cognome']) ?></td>
                    </tr>
                    <tr>
                        <th>Data ingresso</th>
                        <td><?= $dettaglio['data_ingresso'] ?></td>
                    </tr>
                    <tr>
                        <th>Data uscita</th>
                        <td><?= $dettaglio['data_uscita'] ?? '-' ?></td>
                    </tr>
                    <tr>
                        <th>Stato</th>
                        <td style="color:<?= $stato_colori[$dettaglio['stato']] ?? 'black' ?>">
                            <strong><?= htmlspecialchars($dettaglio['stato']) ?></strong></td>
                    </tr>
                    <tr>
                        <th>Costo totale</th>
                        <td><?= $dettaglio['costo_totale'] ? '€ ' . number_format($dettaglio['costo_totale'] / 100, 2) : '-' ?>
                        </td>
                    </tr>
                </tbody>
            </table>
