...
// Utenti meccanico senza profilo associato (per il form nuovo)
$utenti_liberi = [];
if ($azione === 'nuovo') {
...
<!DOCTYPE html>
<html lang="it">
<head><meta charset="UTF-8"><title>Meccanici - Pushin20 Bikes</title></head>
<body>
    ...
    <?php if ($azione === 'lista'): ?>
        ...
        <a href="meccanici.php?azione=nuovo">+ Nuovo Meccanico</a>
        <table border="1" cellpadding="6">
            <tr><th>Nome</th><th>Cognome</th><th>Specializzazione</th><th>€/h</th><th>Username</th><th>Attivo</th><th>Azioni</th></tr>
    ...
    <?php else: ?>
        ...
        <form method="POST">
            ...
            <fieldset>
                ...
                <p>Seleziona un utente meccanico esistente:<br>
            ...
            <div><label>Nome *:</label><br><input type="text" name="nome" value="<?= htmlspecialchars($mec['nome'] ?? '') ?>" required></div>
            <div><label>Cognome *:</label><br><input type="text" name="cognome" value="<?= htmlspecialchars($mec['cognome'] ?? '') ?>" required></div>
            <div><label>Specializzazione:</label><br><input type="text" name="specializzazione" value="<?= htmlspecialchars($mec['specializzazione'] ?? '') ?>"></div>
            <div><label>Costo orario (€) *:</label><br><input type="number" name="costo_orario" value="<?= $mec['costo_orario'] ?? '' ?>" min="1" required></div>
            <div><button type="submit"><?= $azione === 'modifica' ? 'Salva Modifiche' : 'Crea Meccanico' ?></button></div>
        </form>
