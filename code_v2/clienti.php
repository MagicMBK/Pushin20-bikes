...
<!DOCTYPE html>
<html lang="it">
<head><meta charset="UTF-8"><title>Clienti - Pushin20 Bikes</title></head>
<body>
    ...
    <?php if ($azione === 'lista'): ?>
        ...
        <?php if (!$solo_lettura): ?><a href="clienti.php?azione=nuovo">+ Nuovo Cliente</a><?php endif; ?>
        <table border="1" cellpadding="6">
            <tr><th>Nome</th><th>Cognome</th><th>Email</th><th>Telefono</th><th>Username</th><?php if (!$solo_lettura): ?><th>Azioni</th><?php endif; ?></tr>
    ...
    <?php else: ?>
        ...
        <form method="POST">
            ...
            <?php endif; ?>
            <div><label>Nome *:</label><br><input type="text" name="nome" value="<?= htmlspecialchars($cli['nome'] ?? '') ?>" required></div>
            <div><label>Cognome *:</label><br><input type="text" name="cognome" value="<?= htmlspecialchars($cli['cognome'] ?? '') ?>" required></div>
            <div><label>Email *:</label><br><input type="email" name="email" value="<?= htmlspecialchars($cli['email'] ?? '') ?>" required></div>
            <div><label>Telefono:</label><br><input type="text" name="telefono" value="<?= htmlspecialchars($cli['telefono'] ?? '') ?>"></div>
            <div><button type="submit"><?= $azione === 'modifica' ? 'Salva Modifiche' : 'Crea Cliente' ?></button></div>
        </form>
